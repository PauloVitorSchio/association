...
RSpec.describe Person, type: :model do
  ...
  describe '#balance' do
    it 'returns the sum of all debt amounts minus the sum of all payments' do
      person = Person.new(name: 'John Doe', national_id: CPF.generate)
      debt1 = person.debts.build(amount: 100)
      debt2 = person.debts.build(amount: 200)
      debt3 = person.debts.build(amount: 300)

      payment1 = person.payments.build(amount: 100)
      payment2 = person.payments.build(amount: 200)
      payment3 = person.payments.build(amount: 300)

      expect(person.balance).to eq(0)
    end

    it 'returns 0 if there are no debts or payments' do
      person = Person.new(name: 'John Doe', national_id: CPF.generate)
